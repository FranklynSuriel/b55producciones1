...
$query = "
    SELECT p.*, f.numero_factura, f.fecha_emision AS fecha_factura, c.client_name, c.client_company, 
...
if ($pago['condicion_comercial'] === 'Informal') {
    ...
    $itbis2 = $monto * 0.10; // Retención adicional
    $retencion = $itbis + $itbis2;
} else {
...
$fecha_formateada = strftime('%e %b %Y', $timestamp);

// Mostrar la fecha en el PDF
...
$pdf->Cell(0, 10, utf8_decode('Asunto: Certificación de Retención del 100% de ITBIS.'), 0, 1, 'L');
...
// Cuerpo de la carta
$carta = "
BILLY55 PRODUCCIONES SRL, sociedad comercial constituida de acuerdo con las leyes de la República Dominicana, con su domicilio social en el No. 21 de la avenida Theodoro Chasseriau, Sector el Millón, de esta ciudad, con RNC No. 132-92559-9, debidamente representada por su Gerente, Sr. BILLY JOEL SEGURA, portador de la cédula No. 001-1521451-2, por medio de la presente, certifica que mediante {$pago['metodo_pago']} de fecha " . strftime('%e %b %Y', strtotime($pago['fecha_pago'])) . ", por valor de RD$" . number_format($monto_total, 2) . ", procedimos a pagar la factura No. {$pago['numero_factura']} de fecha " . strftime('%e %b %Y', strtotime($pago['fecha_factura'])) . ", por un monto de RD$" . number_format($monto, 2) . ", emitida por {$pago['beneficiario']}, portador de la cédula No. {$pago['cedula_beneficiario']}, por concepto de {$pago['concepto']} del proyecto '{$pago['project_name']}'. por lo cual no se ejecuto retencion alguna. 
