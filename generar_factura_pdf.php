...
$query = "SELECT f.*, c.client_name, c.client_company, c.client_email, c.client_phone,
                 p.project_name, p.global_total, p.imprevisto, p.impuesto
          FROM facturas f
...
// Calcular el total presupuestado como la suma de ITBIS y el valor total
$itbis = $factura['impuesto'];
$valor = $factura['global_total'] + $factura['imprevisto'];
$total_presupuestado = $itbis + $valor;
...
$pdf->Cell(0, 10, utf8_decode('FACTURA - ') . $factura['numero_factura'], 1, 1, 'C', true);
...
$pdf->Cell(0, 6, 'Total Presupuestado: $' . number_format($total_presupuestado, 2), 0, 1, 'R');
